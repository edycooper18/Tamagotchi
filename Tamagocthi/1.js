document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFeed() {
        if (pet.hunger > 90) {
            showMessage(`${pet.name} anceia por mais galaxias.`);
            return;
        }
        applyActionEffects(actionEffects.feed);
        showMessage(`${pet.name} devorou sua casa e por hora está satisfeito!`);
    }

    function handlePlay() {
        if (pet.energy < 20) {
            showMessage(`${pet.name} não quer mais ser entretido por um mortal como você.`);
            return;
        }
    ...
    function handleClean() {
        if (pet.hygiene > 90) {
            showMessage(`${pet.name} não esta mais com parasitas.`);
            return;
        }
        applyActionEffects(actionEffects.clean);
        showMessage(`${pet.name} está pronto para exterminar mais galaxias!`);
    }

    function handleSleep() {
        if (pet.energy > 80) {
            showMessage(`${pet.name} está ativo demais para descançar.`);
            return;
        }
